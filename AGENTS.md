...
1. **[PROJECTS.md](PROJECTS.md) is the project's ground truth.** Read it first,
...
2. **Keep it current in the same change, not later.** A change is not finished
   until `PROJECTS.md` reflects it. CLAUDE.md has the trigger table for what to
   update when.
3. **Precedence: code > `PROJECTS.md` > everything else.** If the code contradicts
   `PROJECTS.md`, the code is right — fix `PROJECTS.md` in the same change.
